...
- **[OpenLMNP](https://github.com/manganate006/openlmnp)** — Open-source, self-hosted French LMNP (furnished-rental) tax accounting with a built-in MCP server: create income/expenses, compute depreciation and fiscal results, and generate the French tax return (liasse 2031) via any MCP-compatible AI assistant. Not an invoicing tool — included for its AI-native/MCP approach to a related accounting workflow. `Free forever` `MCP` `AGPL-3.0`
